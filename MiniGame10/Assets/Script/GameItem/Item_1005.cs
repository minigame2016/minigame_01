...
public class Item_1005 : MonoBehaviour
{
    ...
    {
        // 左右移动
        float rx = Mathf.Sin(Time.time) * Time.deltaTime * (float)GameSystem.Instance.Item_1005_H_Speed;

        // 向下运动
        _transform.Translate(new Vector3(rx, (-(float)GameSystem.Instance.Item_1005_V_Speed * Time.deltaTime)), 0);
    }
