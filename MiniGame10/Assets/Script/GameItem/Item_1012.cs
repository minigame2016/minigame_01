...
public class Item_1012 : MonoBehaviour
{
    ...
    private void ArriveDeadLineOrNot()
    {
        if (this.transform.position.y < TableNum.DeadLine)//飘到屏幕下方了
        {
            ...
            Debug.Log("Item_1012 ArriveDeadLineOrNot Dead");
    ...
    {
        // 左右移动
        float rx = Mathf.Sin(Time.time) * Time.deltaTime * (float)GameSystem.Instance.Item_1012_H_Speed;

        // 向下运动
        _transform.Translate(new Vector3(rx, (-(float)GameSystem.Instance.Item_1012_V_Speed * Time.deltaTime)), 0);
    }
