...
public class Item_1003 : MonoBehaviour
{
    ...
    private void ArriveDeadLineOrNot()
    {
        if (this.transform.position.y < TableNum.DeadLine)//飘到屏幕下方了
        {
            ...
            Debug.Log("Item_1003 ArriveDeadLineOrNot Dead");
    ...
    {
        // 左右移动
        float rx = Mathf.Sin(Time.time) * Time.deltaTime * (float)GameSystem.Instance.Item_1003_H_Speed;

        // 向下运动
        _transform.Translate(new Vector3(rx, (-(float)GameSystem.Instance.Item_1003_V_Speed * Time.deltaTime)), 0);
    }
