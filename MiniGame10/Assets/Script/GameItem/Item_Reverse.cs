...
public class Item_Reverse : MonoBehaviour
{
    ...
    private void ArriveDeadLineOrNot()
    {
        if (this.transform.position.y < TableNum.DeadLine)//飘到屏幕下方了
        {
            ...
            Debug.Log("Item_Reverse ArriveDeadLineOrNot Dead");
        }
    }
    ...
    private void UpdateMove()
    {
        ...
        // 向下运动
        _transform.Translate(new Vector3(rx, (-(float)TableNum.ReverseSpeed * Time.deltaTime)), 0);
    }
    ...
    {
        Debug.Log("Item_Reverse OnClickItemCallback");
        GameSystem.Instance.isClickReverseItem = true;
        GameSystem.Instance.reverseCreateTime = Time.time;
        GameSystem.Instance.reverseEndTime = TableNum.ReverseAddTime;
        GameSystem.Instance.reverseCreTimeScale = Time.timeScale;
        NGUITools.Destroy(_panel_prefab);
    }
}
